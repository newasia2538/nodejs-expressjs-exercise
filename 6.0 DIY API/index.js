...
app.get('/filter', (req, res) => {
  const filteredJoke = jokes.filter( (item) => item.jokeType === req.query.type);
...
app.post('/jokes', (req, res) => {
  try{
    ...
    res.status(400).send(`Something error try again.`);
  }
...
app.patch('/jokes/:id', (req, res) => {
  ...
  try{
    ...
    jokes[index].jokeType = req.body.type || joke.jokeType;
    jokes[index].jokeText = req.body.text || joke.jokeText;
...
app.delete('/jokes/:id', (req, res) => {
  ...
  try{
    ...
    console.log(`Error : ${err}`);
    res.send(`Something when wrong. No jokes were deleted.`);
  }
});
...
app.delete('/all', (req, res) => {
  ...
  try{
    if(key !== masterKey){
      res.status(404).json({error : `Unauthorized user can not access this action.`});
    }else{
      jokes = [];
    ...
    console.log(`Error : ${err}`);
    res.status(404).send(`Something when wrong. No jokes were deleted.`);
  }
});
